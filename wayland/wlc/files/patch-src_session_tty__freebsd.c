--- src/session/tty_freebsd.c.orig	2017-01-08 13:11:37 UTC
+++ src/session/tty_freebsd.c
@@ -0,0 +1,267 @@
+#include <signal.h>
+#include <stdio.h>
+#include <string.h>
...
+   }
+
+   if (ioctl(fd, VT_GETACTIVE, &wlc.old_state.vt) < 0)
+      die("Could not get active vt");
+
...
+	   die("Could not set keyboard mode");
+
...
+   }
+
+   /* Put the tty into raw mode (disable echo output) */
+   struct termios tios;
+   if (tcgetattr(fd, &tios))
+	   die("Could not get tty attr");
+   cfmakeraw(&tios);
+   if (tcsetattr(fd, TCSAFLUSH, &tios))
+	   die("Could not set tty attr");
+
+   struct vt_mode mode = {
+	   .mode = VT_PROCESS,
+	   .relsig = SIGUSR1,
+	   .acqsig = SIGUSR2,
+	   .frsig = SIGIO /* Not used but has to be set to something */
+   };
...
+   bool ret = ioctl(wlc.tty, VT_RELDISP, 1) != -1;
+   return ret;
+}
+
+// Do not call this function directly!
...
+      // However, we need to make sure the vt we initially opened
+	  // is also active on cleanup.
+      // We can't make sure this is synchronized due to unclean exits.
+
+	   int active, current;
+	   ioctl(wlc.tty, VT_GETACTIVE, &active);
+	   ioctl(wlc.tty, VT_GETINDEX, &current);
+	   if(active != current) {
+		   wlc_log(WLC_LOG_INFO, "VT has changed, switch back before cleanup");
+		   if (ioctl(wlc.tty, VT_ACTIVATE, wlc.vt) ||
+			   ioctl(wlc.tty, VT_WAITACTIVE, wlc.vt)) {
+			   wlc_log(WLC_LOG_ERROR, "Failed to activate vt%d for termination", wlc.vt);
+			   goto switchvt;
+		   }
+	   }
+
+	   wlc_log(WLC_LOG_INFO, "Restoring vt %d (0x%lx) (fd %d)",
+			   wlc.vt, wlc.old_state.kb_mode, wlc.tty);
+
...
+		   wlc_log(WLC_LOG_ERROR, "Failed to set vt%d kd_mode to KD_TEXT", wlc.vt);
+
